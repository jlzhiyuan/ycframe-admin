...
//存储poi借口返回对象信息,为气泡重绘使用
var datas = [];
//存储对象属性信息拼接的字符串,传入后台数据库存储模块
var datas1 = [];
//判断气泡显示(隐藏)状态
var judgment =[];
//数据库建表名称,传入后台数据库存储模块
...
PC.init = function() {
	...
	PC.addPoint=function()
	{
		...
		var pcb = function(resultobj){
			var list=resultobj[0].data;
			dbname=resultobj[1].data;//admin_gugong
			//alert(dbname);
				...
				for(var i=0;i<list.length;i++){
				    ...
				    AMap.service(["AMap.PlaceSearch"], function() {
				        ...
				        placeSearch.search(list[i], function(status, result) {
				        	//判断循环进行次数
				        	num1++;
				        	if(num1==list.length && datas.length!=0){
				    	    	//alert("数据已存入数据库");
				    	    	importPoi(datas1);
				    	    	return;
				    	    }
				        	...
				        	if(names.toString().indexOf(name)==-1){				        		
				        		names.push(name);
					    	    ...
					    	    if(result.poiList.pois[0].name!="" && result.poiList.pois[0].location.lng!="" && result.poiList.pois[0].location.lat!="" && result.poiList.pois[0].address!=""){
			...
			var pm = new Lemon.LemonModule("PCStringQuery");
		    pm.addPar("inform",$('#information').val());
	...
	/*PC.addPoint=function(){
		...
		datas = [];
		var names=new Array();
		var list = ["沈阳市","沈阳市沈河区","沈阳市沈河区点中街","沈阳市沈河区点中街171号","沈阳市沈河区点中街12001854号","沈阳市沈河区站点中街","沈阳市沈河区站点中街171号","沈阳市沈河区站点中街12001854号","北京市","北京市东城区","北京市居住区","北京市开放区","北京市缓冲区","北京市北京市区"];
