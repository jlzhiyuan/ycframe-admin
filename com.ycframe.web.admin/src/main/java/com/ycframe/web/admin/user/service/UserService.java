...
public class UserService {
	...
	public boolean savedata(String id, String deptID, String userName, String YHM,
		...
		try {
			...
			manager.load();
			dao = manager.getDao(UserDao.class);
			if ("".equals(deptID) || "".equals(userName) || "".equals(empCode)
				...
				try { 
					...
					if (!"null".equals(id) && id != null && !"".equals(id)) {
						List data1 = dao.findByYHM(userName, id, deptID);
						if (!data1.isEmpty()) { 
							manager.rollback();
						...
						// ,Telepone1=?,Telepone2=?
						int i = dao.updatetryjl(deptID, userName, YHM, sex,
								Telepone1, Telepone2,zy, id);
						if (i > 0) {
							dao.updatery1(userName, empCode);
							// 修改数据
						...
						List data1 = dao.findByYHM(userName, deptID);
						if (!data1.isEmpty()) {
							manager.rollback(); 
							throw new ServiceException("用户名已经存在!");
						}
						...
						if (!"".equals(empCode)) {
							...
							int ii = dao.insertry(ryid, userName, YHM, mm, mm,
									"假数据", Telepone1);
							...
							if (ii > 0) {
								...
								int j = dao.insertryjl(pid, deptID, userName,
